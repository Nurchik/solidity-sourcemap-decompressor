...
class SourceMapLocation:
    ...
    #     if value < -1:
    #         raise ValueError(
    #             "Expected a positive, 0 or -1 offset value but received {}".format(
    #                 value
    #             )
    #         )
    #     self.o = value
    ...
    #     if value < -1:
    #         raise ValueError(
    #             "Expected a positive, 0 or -1 length value but received {}".format(
    #                 value
    #             )
    #         )
    #     self.l = value
    ...
    #     if value < -1:
    #         raise ValueError(
    #             "Expected a positive, 0 or -1 file ID value but received {}".format(
    #                 value
    #             )
    #         )
    #     self.f = value
    ...
    def to_full_component_string(self) -> str:
        return "{}:{}:{}:{}".format(self.offset, self.length, self.file_id, self.jump_type)

    def to_short_component_string(self) -> str:
        return "{}:{}:{}".format(self.offset, self.length, self.file_id)
    ...
    def __eq__(self, other: "SourceMapLocation") -> bool:
        return all(
            (self.offset == other.offset, self.length == other.length, self.file_id == other.file_id, self.jump_type == other.jump_type)
        )

def main():
    acc = [-1, -1, -1, "-"]
    acc_sm = SourceMapLocation(*acc)
    a = time()
    array_length_p = ffi.new("long *")
    res = lib.decompress_opt(source_map.encode(), array_length_p)
    b = time()
    array_length = array_length_p[0]
    c = time()
    rs = ffi.unpack(res, array_length)
    # _rs = [ffi.unpack(i, 4) for i in rs]
    d = time()
    result = [-1] * array_length
    e = time()
    for idx, i in enumerate(rs):
        # if acc != [i.offset, i.size, i.fileId, i.jumpType.decode()]:
        #     acc = [i.offset, i.size, i.fileId, i.jumpType.decode()]
        #     acc_sm = SourceMapLocation(*acc)
        # result[idx] = acc_sm
        result[idx] = SourceMapLocation(*ffi.unpack(i, 4))
    # result = [SourceMapLocation(i.offset, i.size, i.fileId, i.jumpType.decode()) for i in rs]
    f = time()
    print(len(result))
    print(f"Overall run time {f - a} s\nCalling C lib took {b-a} s\nConverting array length took {c-b} s\nunpacking took {d-c} s\narray creation took {e-d} s\nparsing result took {f-e} s")
